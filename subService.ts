...
export async function getSubById(subId: string): Promise<Sub | null> {
  const db = await getDatabase();
  return db.collection<Sub>("subscribers").findOne({ _id: new ObjectId(subId) });
}
...
export async function deleteSubByEmail(email: string): Promise<DeleteResult| null> {
  const db = await getDatabase();
  return db.collection<Sub>("subscribers").deleteOne({email: email });
}

export async function updateSub(subId: string, updateData: Partial<Sub>): Promise<boolean> {
  const db = await getDatabase();
  const result = await db.collection<Sub>("subscribers").updateOne(
...
export async function deleteSub(subId: string): Promise<boolean> {
  const db = await getDatabase();
  const result = await db.collection<Sub>("subscribers").deleteOne({ _id: new ObjectId(subId) });
